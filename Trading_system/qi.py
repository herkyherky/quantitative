...
print('start to test successfully')
...
if not os.path.exists(path):
    os.mkdir(path)
    with open('./data-T/log.txt','a+') as file:
        file.write('0')
        file.close()
...
with open('./data-T/'+timedata+'.txt','a+') as file:
    file.write('')
    file.close()
...
tradingdoce='['+tradingtime+','+','+stock+','+tt+','+str(p)+','+str(equity)+']'

with open('./data/'+str(txt),'a+') as file:
    file.write(tradingdoce)
